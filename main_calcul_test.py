...
def entree_parametre_equation():    
        choix_type = input("l'équation est-elle sous la forme ax**2+bx+c=0?\n")
        if "oui" in choix_type:    
...
def resolution(a, b, c):
    ...
    if delta < 0:
        ...
        print(f"la premiere solution complexe est(-b-i√Δ/2*a):{solution_complexe1}\n")
        print(f"la seconde solution complexe est(-b+i√Δ/2*a):{solution_complexe2}\n")
        ...
        print(f"la première solution de l'équation est(-b-√Δ/2*a): {solution2}\n")
        solution4 = (-b+racine_de_delta)/(2*a)
        print(f"la seconde solution est(-b-√Δ/2*a): {solution4}\n")
